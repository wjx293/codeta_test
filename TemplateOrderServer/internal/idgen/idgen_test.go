...
func TestGenerator_NewSeeded(t *testing.T) {
	today := time.Now().Format("20060102")
	g := NewSeeded(3)
	no := g.GenerateOrderNo()
	if no != fmt.Sprintf("TO%s%010d", today, 4) {
		t.Fatalf("expected next seq 4, got %s", no)
	}
}

func TestParseSeqFromNo(t *testing.T) {
	today := time.Now().Format("20060102")
	no := fmt.Sprintf("TO%s%010d", today, 42)
	if got := ParseSeqFromNo("TO", no); got != 42 {
		t.Fatalf("expected 42, got %d", got)
	}
	if got := ParseSeqFromNo("TO", "invalid"); got != 0 {
